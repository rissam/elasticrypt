...
/**
  ...
  */
class EncryptedTranslogStream(pageSize: Int, keyProvider: KeyProvider) extends ChecksummedTranslogStream {
  ...
  override def openInput(translogFile: File): StreamInput = {
    val encryptedFileInputStream = new ChannelInputStream(new EncryptedFileChannel(translogFile, pageSize, keyProvider))
    var success = false
    try {
